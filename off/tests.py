...
class StaticPageTest(TestCase):
    ...
    def test_savedProducts_page(self):
        response = self.client.get('/produit/1')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'off/product.html')
        self.assertEqual(response.resolver_match.func.__name__, 'product')
        self.assertIn('Repères nutritionnels pour 100g', response.content.decode('utf8'))
